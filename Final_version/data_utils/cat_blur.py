...
class cat_face_blur(object):
    ...
    def __init__(self, blur_size=0.7, Gaussian_blur=20, path_to_blur='to_blur/', extension='.jpg'):
        ...
        self.files = Catface_jpg = [f for f in os.listdir(
            os.getcwd() + '/' + self.path_to_blur) if f.endswith(self.extension)]
        self.dim = len(self.files)
        print('*********************************************')
        print('Total Images to blur: ' + str(self.dim))
        print('The blur will cover' +
              str(self.blur_size * 100) + '%' + 'of the images')
    ...
    def main(self):
        ...
        if not self.blur_size < 0 and not self.blur_size > 1 and not self.Gaussian_blur < 0 and not self.Gaussian_blur > 100:
            for i in range(self.dim):
                cat = Image.open(os.getcwd() + '/' +
                                 self.path_to_blur + self.files[i])
                ...
                crop_cat = crop_cat.filter(
                    ImageFilter.GaussianBlur(self.Gaussian_blur))
